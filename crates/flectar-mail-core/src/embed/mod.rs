...
//! (BGE / MiniLM). No ONNX runtime, no native ML libs - Flectar Mail stays a single
...
impl LocalCandle {
    ...
    pub(crate) fn load_preverified(dir: &Path, spec: &'static ModelSpec) -> Result<Self> {
        ...
        // private cache directory. Flectar never mutates the file in place;
...
mod tests {
    ...
    // Requires network + ~130MB download; run with `cargo test -p flectar-mail-core
    // -- --ignored embed_smoke`.
    #[tokio::test]
    #[ignore]
    async fn embed_smoke() {
        let tmp = std::env::temp_dir().join("flectar-mail-embed-test");
