...
impl TokenProvider {
    ...
    pub async fn store_initial(
        ...
        if let Some(rt) = refresh_token {
            credentials::store_async(self.credentials.clone(), account_id, Slot::RefreshToken, rt)
                .await?;
        }
    ...
    pub async fn access_token_for_scope(
        ...
        let (client_id, client_secret) = self.registration(account_id, provider).await?;
        let refresh_token =
    ...
    async fn refresh(&self, account_id: i64, provider: Provider) -> Result<CachedToken> {
        ...
        if provider == Provider::Gmail
            && let Some(scopes) = platform_google_scopes(&refresh_token)
        {
    ...
    async fn registration(
        ...
        match credentials::load_async(self.credentials.clone(), account_id, Slot::OAuthClientId)
            .await
        {
            Ok(client_id) if !client_id.trim().is_empty() => {
                let client_secret = credentials::load_async(
                    self.credentials.clone(),
                    account_id,
                    Slot::OAuthClientSecret,
                )
            ...
            // Accounts created before per-account OAuth registrations were
            // introduced continue to use the configured/bundled registration.
            _ => crate::oauth::providers::resolve_credentials(provider),
        }
    }
